...
const FIRST_FIRE_MS    = 60 * 1000;

// Last-seen timestamp per participant to skip re-broadcasting stale positions
...
// Pull a named <Data name="X"><value>Y</value></Data> from a KML snippet
...
// MapShare returns Placemarks newest-first, so we take the first one.
function parseKml(kml) {
  ...
  // Garmin format: "07/16/2023 10:30:00" (UTC) — append timezone so Date parses correctly
...
// Normalize user-supplied URL to the KML feed endpoint.
// Accepts: bare username, share.garmin.com/username, full Feed/Share URL.
function normalizeFeedUrl(raw) {
  ...
  // Fallback — return as-is and let the HTTP request fail visibly
  return s;
}
...
async function pollParticipant(participant) {
  ...
  // Request only the last 24 hours to keep the payload small
  ...
  try {
    ...
    logger.log('inreach', 'warn', `Poll failed for ${participant.name} (#${participant.bib}): ${e.message}`);
  }
}

async function pollAll() {
  const activeRaceIds = db.prepare("SELECT id FROM races WHERE status='active'").all().map(r => r.id);
